...
(function($){
	...
	$.addFlex = function(t,p)
	{
		...
		var g = {
					 ...
					 function(i,row) 
					 	{
							...
							tr.setAttribute('style', "cursor:pointer;cursor:hand;");
							if (i % 2 && p.striped) tr.className = 'erow';
							
							if (row.id) tr.id = 'row' + row.id;
							var url_target_request = replaceAll(document.location.href, 'myaccount/order_list', 'x_page.php');
							tr.setAttribute('onClick', "makeRequestPop('"+url_target_request+"', 'process_container','page=x_detail_order&page_required=1&act=show&id="+row.id+"');$.fn.colorbox({width:'1020px', height:'80%', inline:true, href:'#process_container'});");
							//add cell
				 	...
				 	function ()
						{
							...
							var tr = document.createElement('tr');
							//tr.setAttribute('onClick', "alert('dafadfad')"); 
							if (i % 2 && p.striped) tr.className = 'erow';
			...
			populate: function () { //get latest data
				...
				var param = [
					...
					,{ name : 'rp', value : p.rp }
					,{ name : 'cust_id', value : p.cust_id }
					,{ name : 'where_date', value : p.where_date }
		...
		if (p.colModel)
		{
			...
			//tr.setAttribute('onClick', "alert('dua')"); 
		...
		$('#reload_grid').click(function(){g.populate()});
			 	...
			 	function ()
					{
						...
						$('tbody',g.nDiv).append('<tr><td class="ndcol1"><input type="checkbox" '+ chk +' class="togCol" value="'+ cn +'" />ddd</td><td class="ndcol2">'+this.innerHTML+'</td></tr>');
						cn++;
					}
			);
